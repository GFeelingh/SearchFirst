...
(() => {
  ...
  const stopSouBaObservers = () => {
    ...
    if (pauseObserver) {
      try { pauseObserver.disconnect(); } catch (_) {}
      pauseObserver = null;
    }
  };

  const failOpen = (reason) => {
    console.error('[SearchFirst] 初始化失败，已自动放行原页面：', reason);
    ...
    try { document.getElementById('souba-buffer')?.remove(); } catch (_) {}
    try { document.getElementById('souba-style')?.remove(); } catch (_) {}
    try { document.getElementById('souba-early-style')?.remove(); } catch (_) {}
  ...
  // 首屏遮罩启用后若 5 秒内仍未创建 buffer，自动放行宿主网站。
  startupWatchdog = setTimeout(() => {
    if (!userEnteredFeed && !document.getElementById('souba-buffer')) {
      failOpen(new Error('startup watchdog: buffer was not created'));
    }
  }, 5000);
  ...
  const install = () => {
    ...
    const platformIcons = sites.map(s => {
      ...
      return el('span', {
        ...
        'data-key': s.key,
        title: s.label
      }, [el('img', {src: icons[s.key] || '', alt: ''})]);
    });

    const appearance = el('div', {id: 'souba-appearance'}, [
      el('div', {id: 'souba-segment'}, [
        el('button', {id: 'souba-site-mode', type: 'button', text: '跟随网站'}),
        el('button', {id: 'souba-custom-mode', type: 'button', text: '自定义'})
      ]),
      el('button', {id: 'souba-palette', type: 'button', 'aria-label': '选择自定义颜色'}, [
        el('span', {id: 'souba-palette-dot'})
      ])
    ]);
    ...
    const swatchNodes = swatchData.map(([bg, title]) => {
      const swatch = el('button', {
        className: 'souba-swatch',
        'data-bg': bg,
        title
      });
      swatch.style.setProperty('--sw', bg);
      return swatch;
    });
    const popoverNode = el('div', {id: 'souba-popover'}, [
      el('div', {className: 'souba-pop-title', text: '背景色调'}),
      el('div', {id: 'souba-swatches'}, swatchNodes),
      el('label', {id: 'souba-custom-row'}, [
        el('span', {text: '自定义背景'}),
        el('input', {id: 'souba-color', type: 'color'})
      ])
    ]);

    const currentSite = el('div', {id: 'souba-site'}, [
      el('img', {
        id: 'souba-current-logo',
        'data-key': site.key,
        src: icons[site.key] || '',
        alt: ''
      }),
      el('span', {text: site.label})
    ]);
    const searchIcon = svgEl('svg', {
      id: 'souba-search-icon',
      viewBox: '0 0 24 24',
      fill: 'none',
      'stroke-width': '1.9',
    ...
    searchIcon.append(
      ...
      svgEl('path', {d: 'm20 20-3.6-3.6'})
    );
    const searchWrap = el('div', {id: 'souba-search-wrap'}, [
      searchIcon,
      el('input', {
        id: 'souba-search',
        type: 'text',
      ...
      el('div', {id: 'souba-enter', text: 'Enter'})
    ]);
    const browseButton = el('button', {id: 'souba-browse', type: 'button'}, [
      el('span', {id: 'souba-browse-arrow', text: '↓'}),
      '我就是想刷'
    ]);
    const help = el('div', {id: 'souba-help'}, [
      '滚轮下滑、',
      el('span', {className: 'souba-key', text: 'Space'}),
      '、',
      el('span', {className: 'souba-key', text: 'PgDn'}),
      ' 或 ',
      el('span', {className: 'souba-key', text: '↓'}),
      ' 可直接进入原首页'
    ]);
    const main = el('main', {id: 'souba-main'}, [
      el('section', {id: 'souba-content'}, [
        currentSite,
        el('h1', {id: 'souba-title', text: '你原本想搜什么？'}),
        searchWrap,
        el('div', {id: 'souba-browse-area'}, [browseButton, help])
      ])
    ]);
    const footer = el('footer', {id: 'souba-footer'}, [
      el('div', {id: 'souba-platform-strip'}, [
        el('span', {id: 'souba-platform-label', text: '支持平台'}),
        el('div', {id: 'souba-platforms'}, platformIcons)
      ])
    ]);

    // SearchFirst UI 全部通过 DOM API 构造，避免触发 YouTube Trusted Types。
    box.append(appearance, popoverNode, main, footer);
    ...
    // 键盘退出必须独占触发退出的那一次物理按键。
    // 否则 ArrowDown / PageDown / Space / Escape 在 SouBa 退出后还会继续
    // 传给原网站，例如抖音会把同一个 ArrowDown 当作“下一条视频”。
    const EXIT_KEYS = new Set(['PageDown',' ','ArrowDown','Escape']);
    let consumedExitKey = null;

    const exitKeyId = e => e.code || e.key;

    const consumeExitKey = e => {
      e.preventDefault();
      e.stopPropagation();
    ...
    const keyUpHandler = e => {
      if (!consumedExitKey || exitKeyId(e) !== consumedExitKey) return;

      // 某些网站把快捷键绑定在 keyup，因此对应释放事件也要吞掉。
      consumeExitKey(e);
      consumedExitKey = null;
      window.removeEventListener('keyup', keyUpHandler, true);

      // 此时触发退出的实体键已经真正松开，可以安全移除键盘监听。
      if (userEnteredFeed) {
    ...
    const keyHandler = e => {
      const id = exitKeyId(e);

      // leave() 已经执行后，继续吞掉长按该键产生的 repeat keydown，
      // 直到收到同一个实体键的 keyup。
      if (consumedExitKey && id === consumedExitKey) {
        consumeExitKey(e);
        return;
      }

      if (userEnteredFeed) {
        // 非键盘方式退出时，没有待释放按键，下一次键盘事件到来即可清理。
        if (!consumedExitKey) {
          window.removeEventListener('keydown', keyHandler, true);
        }
        return;
      ...
      // 保持现有输入框规则：
      // Space / ArrowDown 属于输入框；PageDown / Escape 仍可退出。
      if (document.activeElement === input && !['PageDown','Escape'].includes(e.key)) return;

      if (!EXIT_KEYS.has(e.key)) return;

      // 在 window capture 阶段彻底消费事件，再退出 SouBa。
      consumeExitKey(e);
      consumedExitKey = id;
