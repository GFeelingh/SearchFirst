...
(() => {
  ...
  // 搜索跳转保护 -------------------------------------------------------------
  // 完整页面搜索导航会销毁旧 SearchFirst。目标搜索页的新 document 需要在
  ...
  if (isLikelySearchDestination) {
    ...
    transitionStyle.id = 'searchfirst-search-transition-style';
    transitionStyle.textContent = `
      html.searchfirst-search-transition {
        background:${transitionBg} !important;
      }
      html.searchfirst-search-transition::before {
        content:"";
        position:fixed;
        inset:0;
        z-index:2147483647;
    ...
    document.documentElement.classList.add('searchfirst-search-transition');
    ...
    const removeTransition = () => {
      ...
      document.documentElement.classList.remove('searchfirst-search-transition');
    ...
    // 目标搜索页只负责过渡保护，不再创建完整 SearchFirst。
    return;
  }

  // 搜索子域名（search.bilibili.com / s.weibo.com / so.toutiao.com 等）
  // 即使路径恰好是 /，也不能重新盖上 SearchFirst。
  if (!site.names.includes(host) || !site.home()) return;

  // 防止同时启用稳定版和 Beta 时重复插入。
  if (document.documentElement.dataset.searchfirstActive === '1') return;
  document.documentElement.dataset.searchfirstActive = '1';
  ...
  earlyStyle.id = 'searchfirst-early-style';
  earlyStyle.textContent = `
    html.searchfirst-preveil { background:${earlyBg} !important; }
    html.searchfirst-preveil::before{
  ...
  document.documentElement.classList.add('searchfirst-preveil');
  ...
  const samplePageBackground = (fallback) => {
    // 不读取 html：html 上已经挂了 SearchFirst 的预遮罩，会污染检测。
    const candidates = [
      document.body,
  ...
  const install = () => {
    if (userEnteredFeed || installing || document.getElementById('searchfirst-buffer')) return;
    if (!document.body) {
    ...
    style.textContent = `
      ...
      #searchfirst-buffer.searchfirst-leave{transform:translateY(-100%);opacity:.98;pointer-events:none}
      #searchfirst-buffer.searchfirst-theme-animate{transition:transform .28s cubic-bezier(.2,.8,.2,1),opacity .28s ease,background-color .18s ease,color .18s ease}
      #searchfirst-main{display:flex;align-items:center;justify-content:center;padding:62px 24px 30px}
      #searchfirst-content{width:min(650px,100%);transform:translateY(-4vh)}
      #searchfirst-site{display:inline-flex;align-items:center;gap:9px;margin-bottom:24px;color:var(--secondary);font-size:14px;font-weight:500}
      #searchfirst-site img{width:23px;height:23px;object-fit:contain}
      #searchfirst-title{margin:0 0 26px;font-size:clamp(36px,4.6vw,50px);line-height:1.08;letter-spacing:-.042em;font-weight:700;text-align:left}
      #searchfirst-search-wrap{position:relative;width:100%}
      #searchfirst-search-icon{position:absolute;left:20px;top:50%;transform:translateY(-50%);width:20px;height:20px;stroke:var(--secondary);pointer-events:none}
      #searchfirst-search{box-sizing:border-box;width:100%;height:64px;padding:0 102px 0 54px;border:1px solid var(--line);border-radius:18px;background:color-mix(in srgb,var(--surface) 92%,transparent);color:var(--text);outline:none;font-size:18px;box-shadow:0 1px 1px rgba(0,0,0,.015);transition:border-color .15s ease,box-shadow .15s ease,background .15s ease}
      #searchfirst-search::placeholder{color:var(--quiet)}
      #searchfirst-search:focus{background:var(--surface);border-color:var(--accent);box-shadow:0 0 0 4px var(--focus)}
      #searchfirst-enter{position:absolute;right:14px;top:50%;transform:translateY(-50%);min-width:54px;height:34px;padding:0 11px;display:flex;align-items:center;justify-content:center;border:1px solid color-mix(in srgb,var(--text) 13%,transparent);border-radius:9px;background:color-mix(in srgb,var(--surface) 92%,transparent);color:var(--secondary);font-size:12px;user-select:none}
      #searchfirst-browse-area{margin-top:34px;display:flex;flex-direction:column;align-items:flex-start;gap:4px}
      #searchfirst-browse{margin-left:-2px;border:0;background:transparent;color:var(--secondary);padding:7px 2px;font:inherit;font-size:15px;cursor:pointer}
      #searchfirst-browse:hover{color:var(--text)}
      #searchfirst-browse-arrow{margin-right:6px;color:var(--quiet)}
      #searchfirst-help{color:var(--quiet);font-size:12px;line-height:1.5}
      .searchfirst-key{display:inline-flex;align-items:center;justify-content:center;min-width:18px;height:18px;padding:0 4px;border:1px solid color-mix(in srgb,var(--text) 10%,transparent);border-bottom-color:color-mix(in srgb,var(--text) 16%,transparent);border-radius:5px;background:color-mix(in srgb,var(--surface) 48%,transparent);color:var(--quiet);font-size:10px;vertical-align:1px}
      #searchfirst-footer{padding:0 24px 20px}
      #searchfirst-platform-strip{max-width:980px;margin:0 auto;border-top:1px solid color-mix(in srgb,var(--text) 6%,transparent);padding-top:16px;display:flex;align-items:center;justify-content:center;gap:11px}
      #searchfirst-platform-label{color:var(--quiet);font-size:11px;white-space:nowrap}
      #searchfirst-platforms{display:flex;align-items:center;justify-content:center;flex-wrap:wrap;gap:12px}
      .searchfirst-platform{width:20px;height:20px;display:grid;place-items:center;opacity:.42;transition:opacity .14s ease,transform .14s ease,filter .14s ease}
      .searchfirst-platform:hover{opacity:.86;transform:translateY(-1px)}
      .searchfirst-platform img{width:17px;height:17px;object-fit:contain;display:block}
      #searchfirst-buffer[data-theme-dark="1"] .searchfirst-platform.searchfirst-light-sensitive{opacity:.62}
      #searchfirst-buffer[data-theme-dark="1"] .searchfirst-platform.searchfirst-monochrome img{filter:invert(1) brightness(1.15)}
      #searchfirst-buffer[data-theme-dark="1"] .searchfirst-platform.searchfirst-monochrome:hover img{filter:invert(1) brightness(1.35)}
      #searchfirst-buffer[data-theme-dark="1"] #searchfirst-current-logo[data-key="x"],
      #searchfirst-buffer[data-theme-dark="1"] #searchfirst-current-logo[data-key="threads"]{filter:invert(1) brightness(1.18)}

      #searchfirst-appearance{position:absolute;top:18px;right:20px;z-index:5;display:flex;align-items:center;gap:8px}
      #searchfirst-segment{display:flex;padding:3px;border:1px solid color-mix(in srgb,var(--text) 9%,transparent);background:var(--overlay);border-radius:12px;backdrop-filter:blur(10px)}
      #searchfirst-segment button{height:29px;border:0;border-radius:9px;background:transparent;color:var(--secondary);padding:0 11px;font:inherit;font-size:12px;cursor:pointer}
      #searchfirst-segment button.active{background:var(--surface);color:var(--text);font-weight:600;box-shadow:0 1px 4px rgba(0,0,0,.07)}
      #searchfirst-palette{display:none;width:36px;height:36px;border-radius:11px;border:1px solid color-mix(in srgb,var(--text) 9%,transparent);background:var(--overlay);cursor:pointer;place-items:center;backdrop-filter:blur(10px)}
      #searchfirst-palette.show{display:grid}
      #searchfirst-palette-dot{width:15px;height:15px;border-radius:50%;background:var(--bg);border:1px solid color-mix(in srgb,var(--text) 18%,transparent)}
      #searchfirst-popover{position:absolute;top:62px;right:20px;z-index:6;width:250px;padding:14px;border-radius:16px;background:color-mix(in srgb,var(--surface) 96%,transparent);border:1px solid color-mix(in srgb,var(--text) 9%,transparent);box-shadow:0 18px 46px rgba(0,0,0,.11);display:none}
      #searchfirst-popover.open{display:block}
      .searchfirst-pop-title{font-size:12px;font-weight:600;color:var(--text);margin-bottom:10px}
      #searchfirst-swatches{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}
      .searchfirst-swatch{width:35px;height:35px;padding:3px;border-radius:50%;border:2px solid transparent;background:transparent;cursor:pointer}
      .searchfirst-swatch::before{content:"";display:block;width:100%;height:100%;border-radius:50%;background:var(--sw);border:1px solid rgba(0,0,0,.10)}
      .searchfirst-swatch.active{border-color:var(--accent)}
      #searchfirst-custom-row{margin-top:12px;padding-top:11px;border-top:1px solid color-mix(in srgb,var(--text) 8%,transparent);display:flex;align-items:center;justify-content:space-between;color:var(--secondary);font-size:12px}
      #searchfirst-color{width:34px;height:26px;padding:0;border:0;background:transparent;cursor:pointer}

      @media(max-width:700px){
        #searchfirst-main{padding-left:18px;padding-right:18px}
        #searchfirst-content{transform:translateY(-2vh)}
        #searchfirst-title{font-size:38px}
        #searchfirst-search{height:60px}
      }
    `;
    ...
    const platformIcons = sites.map(s => {
      const classes = [
        'searchfirst-platform',
        monochromeKeys.has(s.key) ? 'searchfirst-monochrome' : '',
        lightSensitiveKeys.has(s.key) ? 'searchfirst-light-sensitive' : ''
      ].filter(Boolean).join(' ');
      return `<span class="${classes}" data-key="${s.key}" title="${s.label}"><img src="${icons[s.key] || ''}" alt=""></span>`;
    }).join('');

    box.innerHTML = `
      <div id="searchfirst-appearance">
        <div id="searchfirst-segment">
          <button id="searchfirst-site-mode" type="button">跟随网站</button>
          <button id="searchfirst-custom-mode" type="button">自定义</button>
        </div>
        <button id="searchfirst-palette" type="button" aria-label="选择自定义颜色">
          <span id="searchfirst-palette-dot"></span>
        </button>
      </div>

      <div id="searchfirst-popover">
        <div class="searchfirst-pop-title">背景色调</div>
        <div id="searchfirst-swatches">
          <button class="searchfirst-swatch" data-bg="#f3f6f4" style="--sw:#f3f6f4" title="雾青"></button>
          <button class="searchfirst-swatch" data-bg="#f7f6f2" style="--sw:#f7f6f2" title="柔白"></button>
          <button class="searchfirst-swatch" data-bg="#f2f5f8" style="--sw:#f2f5f8" title="雾蓝"></button>
          <button class="searchfirst-swatch" data-bg="#f5f5f7" style="--sw:#f5f5f7" title="中性灰"></button>
          <button class="searchfirst-swatch" data-bg="#f1f5f0" style="--sw:#f1f5f0" title="淡鼠尾草"></button>
        </div>
        <label id="searchfirst-custom-row">
          <span>自定义背景</span>
          <input id="searchfirst-color" type="color">
        </label>
      </div>

      <main id="searchfirst-main">
        <section id="searchfirst-content">
          <div id="searchfirst-site">
            <img id="searchfirst-current-logo" data-key="${site.key}" src="${icons[site.key] || ''}" alt="">
            <span>${site.label}</span>
          </div>

          <h1 id="searchfirst-title">你原本想搜什么？</h1>

          <div id="searchfirst-search-wrap">
            <svg id="searchfirst-search-icon" viewBox="0 0 24 24" fill="none" stroke-width="1.9" stroke-linecap="round">
              <circle cx="11" cy="11" r="7"></circle>
              <path d="m20 20-3.6-3.6"></path>
            </svg>
            <input id="searchfirst-search" type="text" autocomplete="off" spellcheck="false" placeholder="输入关键词…">
            <div id="searchfirst-enter">Enter</div>
          </div>

          <div id="searchfirst-browse-area">
            <button id="searchfirst-browse" type="button"><span id="searchfirst-browse-arrow">↓</span>我就是想刷</button>
            <div id="searchfirst-help">
              滚轮下滑、<span class="searchfirst-key">Space</span>、<span class="searchfirst-key">PgDn</span> 或 <span class="searchfirst-key">↓</span> 可直接进入原首页
            </div>
          </div>
        </section>
      </main>

      <footer id="searchfirst-footer">
        <div id="searchfirst-platform-strip">
          <span id="searchfirst-platform-label">支持平台</span>
          <div id="searchfirst-platforms">${platformIcons}</div>
        </div>
      </footer>
    `;
    ...
    const input = box.querySelector('#searchfirst-search');
    const siteModeBtn = box.querySelector('#searchfirst-site-mode');
    const customModeBtn = box.querySelector('#searchfirst-custom-mode');
    const paletteBtn = box.querySelector('#searchfirst-palette');
    const popover = box.querySelector('#searchfirst-popover');
    const colorPicker = box.querySelector('#searchfirst-color');
    const swatches = [...box.querySelectorAll('.searchfirst-swatch')];
    ...
    const applyAppearance = ({animate = false} = {}) => {
      ...
      box.classList.toggle('searchfirst-theme-animate', Boolean(animate));
      applyThemeVars(t);
    ...
    const stopSearchFirstObservers = () => {
      if (rootObserver) {
    ...
    const removeSearchFirst = ({animate = true, resumeMedia = true} = {}) => {
      stopSearchFirstObservers();
      document.documentElement.classList.remove('searchfirst-preveil');

      if (resumeMedia) restoreMediaPlay();

      const finish = () => {
        box.remove();
        document.getElementById('searchfirst-style')?.remove();
        document.getElementById('searchfirst-early-style')?.remove();
        delete document.documentElement.dataset.searchfirstActive;
      };

      if (animate) {
        box.classList.add('searchfirst-leave');
    ...
    const leave = () => {
      ...
      installing = false;
      removeSearchFirst({animate:true, resumeMedia:true});
    ...
    const searchCurrentSite = () => {
      ...
      const revealSpaResult = () => {
        ...
        cleanupSearchWatch();
        GM_setValue('pendingSearchTransition', '');
        removeSearchFirst({animate:false, resumeMedia:true});
      };
      ...
      const searchMutationObserver = new MutationObserver(mutations => {
        ...
        // 只统计搜索路由发生后的真实页面变化。
        // SearchFirst 自己位于 html 直属层，过滤掉它和自己的 style。
        const meaningful = mutations.some(mu => {
          const t = mu.target;
          if (!t) return false;
          if (t === box || box.contains?.(t)) return false;
          if (t.id === 'searchfirst-style' || t.id === 'searchfirst-early-style') return false;
          return true;
        });
      ...
      setTimeout(() => {
        ...
        if (location.href === before && document.visibilityState === 'visible') {
          ...
          rootObserver = new MutationObserver(() => {
            if (userEnteredFeed) return;
            if (!document.getElementById('searchfirst-buffer')) install();
            pauseAll();
          });
    ...
    box.querySelector('#searchfirst-browse').addEventListener('click', leave);
    ...
    // 这样 X/YouTube/Reddit 等 SPA 加载初期的临时黑白闪烁不会被 SearchFirst 放大。
